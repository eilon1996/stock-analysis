...
from fillter_and_find_old_app import calculation
...
class FinanceProduct:
    ...
    def __init__(self, symbol):
        ...
        try:
            ...
            ###### get stock info ######
            info = ticker.info
            # print(ticker, ": sector - ", info["sector"], ", industry - ", info["industry"])
            print("industry")
        except:
        ...
        try:
            ...
            self.price_history = history.Open.values_per_quarter[:-1]  # the last one is today price
            not_nan = self.price_history == self.price_history
            self.price_history = self.price_history[not_nan]

            price_history = np.asarray([self.price_history, history.index.year.values_per_quarter[::2], history.index.month.values_per_quarter[::2]])
        ...
        try:
            ...
            dividends = np.asarray([dividends.values_per_quarter, dividends.index.year, dividends.index.month])

            dividends = ticker.dividends
            dividends, dates = dividends.values_per_quarter, list(dividends.index.year)
...
if __name__ == '__main__':
    ...
    yfinance.Ticker
    object < MSFT >: sector - Technology, industry - Software—Infrastructure
    yfinance.Ticker
    object < AAPL >: sector - Technology, industry - Consumer
    Electronics"""
    fp = FinanceProduct("qqq")
    fp = FinanceProduct("SOXX")
